...
Semantic HTML is code that describes its content, so you can immediately see and understand what it does.
Non-semantic HTML doesn't describe its content.

2. Name two big differences between ```display: block;``` and ```display: inline;```.

```display: block;``` starts on a new line, and takes up the whole width. You can use padding and margin to move stuff around.

```display: inline;``` doesn't start on a new line. This is useful in cases where you don't want to start a new line. If you wanted text to be bold, and used the <span> tags, for example.
...
- Version Control: Easily compare changes, and revert back to older code in case of an emergency.
- Non-linear updates: It's easy for several people/teams to work on different parts of one project, while still having one main repo.
- Easy to fork: It's really simple to look at the code of a repo, download locally, and edit as wanted or needed. (Plus, a lot of software on GitHub is FOSS.)
